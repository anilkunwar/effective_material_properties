...
def main():
    ...
    st.latex(r"\rho_{\phi} = A_{\phi}  \cdot (T - T_{\text{ref}}) + B_{\phi}  \cdot (T - T_{\text{ref}})^2 + C \cdot (T_{\phi}  - T_{\text{ref}})^3 + D_{\phi}  \cdot \ln(E_{\phi}  \cdot (T - T_{\text{ref}})) + F_{\phi} )")
    ...
    for i in range(num_phases):
        ...
        E = st.sidebar.number_input(f"E_{phase_name}", value=1.0, key=f"{key_prefix}_E")
    ...
    for phase in phases:
        ...
        rho_values = [generic_density(T, A, B, C, D, E, F, Tref, Tmin_phase, Tmax_phase) for T in T_values]
        ax.plot(T_values, rho_values, label=phase_name, color=color, linewidth=linewidth)
